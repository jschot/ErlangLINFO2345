...
initformeasure(ID) ->
    ...
    Table = lists:sublist(ShuffledT, V),
    ets:insert(x, {ID, Table}),
    ets:insert(known_nodes, {ID, [Node || {_, Node} <- Table]}),
...
rpswithdiscovery(ID) ->
    ...
    [{_,L}] = ets:lookup(x, l),
    [{_,R}] = ets:lookup(x, r),
    RandomToAdvertise = trunc(L*R),
    % Items to advertise
    ShuffledT = [Y||{_,Y} <- lists:sort([{rand:uniform(), N} || N <- TableInc])],
    REntries = lists:sublist(ShuffledT, L-RandomToAdvertise-1) ++ [{0,ID}],
    % Get random entries
    [{_, KnownSet}] = ets:lookup(known_nodes, ID),
    LeftNodeSet = sets:subtract(sets:from_list(KnownSet), sets:from_list([Node || {_, Node} <- REntries])),
    RandomNodes = lists:sublist(sets:to_list(LeftNodeSet), RandomToAdvertise),
    %Reset to zero the age of Q
    Table2 = lists:delete({AgeQ,Q}, TableInc),
    TableOK = Table2 ++ [{0,Q}],
    ets:insert(x, {ID, TableOK}),
    ...
    %Send the l-1 entries to Q
    ToAdv = REntries ++ [{0, Node} || Node <- RandomNodes],
    % io:fwrite("Q : ~w, ID : ~w, ToAdv : ~w~n", [Q, ID, ToAdv]),
    Q ! {advertise, ID, ToAdv}.
...
rcv(ID) ->
    receive
        {advertise, From, Entries} ->
            ...
            % Update known set
            [{_, KnownSet}] = ets:lookup(known_nodes, ID),
            NewKnownSet = sets:union(sets:from_list(KnownSet), sets:from_list(EntriesWoP)),
            ets:insert(known_nodes, {ID, sets:to_list(NewKnownSet)}),
