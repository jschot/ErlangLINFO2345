...
print(discoverednodes[99][turn[99]])
"""
for i in discoverednodes.keys() :
    print("Noeud {} :  {}N - {}% en {} turn".format(i,len(discoverednodes[i][turn[i]]),(len(discoverednodes[i][turn[i]])/500)*100,turn[i]))
...
for i in range(1,max(turn.values())):
    ...
    for j in range(1,max(turn.keys())) :
        if turn[j] >= i :
            cnt += 1
            sum += (len(discoverednodes[j][i])/500)*100
    avg = sum/cnt
    y.append(avg)
